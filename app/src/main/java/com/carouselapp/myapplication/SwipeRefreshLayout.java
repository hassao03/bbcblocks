...
/**
 * A copy and paste of SwipeRefreshLayout v1.1.0
 * with some modifications allowing us to extract swipe progress.
 */
public class SwipeRefreshLayout extends ViewGroup implements NestedScrollingParent3,
        ...
        NestedScrollingChild, SwipeRefreshView {

    /** ---- Chameleon changes ---- **/
    /**
     * Any references in SwipeRefreshLayout to the following are to support Chameleon on Chrysalis
     */
    ...
    public void setOnProgressListener(OnProgressListener listener){
        ...
        swipeState = new SwipeState(mOnProgressListener);
    }
    ...
    public static class SwipeState {
        ...
        public SwipeState(OnProgressListener listener) {
            this.listener = listener;
        }

        void setSwiping(boolean swiping) {
            if (swiping && !this.swiping) {
...
//        mCircleView.setImageDrawable(mProgress);
    ...
    private void createProgressView() {
        ...
        mCircleView.setBackgroundColor(Color.WHITE);
...
//        mCircleView.setImageDrawable(mProgress);
        ...
        // Chameleon change: We don't want to see the swipe refresh progress spinner
     ...
     * gesture.
     */
    public void setOnRefreshCustomListener(@NotNull androidx.swiperefreshlayout.widget.SwipeRefreshLayout.OnRefreshListener listener) {
        mListener = listener;
    }
...
//        mCircleView.setScaleY(progress);
    ...
    private void runBounce(){
        ValueAnimator valueAnimator = ValueAnimator.ofFloat(mCurrentTargetOffsetTop/(mTotalDragDistance - mOriginalOffsetTop), 0);
        valueAnimator.setDuration(500);
        valueAnimator.setInterpolator(new DecelerateInterpolator());
        valueAnimator.addUpdateListener(animation -> {
            float animatedValue = (float) animation.getAnimatedValue();
            if(animatedValue == 0){
                mOnProgressListener.onSwipeCancel();
                swipeState.setSwiping(false);
                setEnabled(true);
            } else {
                mOnProgressListener.onProgress(animatedValue);
            }
        });
...
//        if (mTarget instanceof ContentView && ((ContentView) mTarget).getChildCount() > 0){
//            return ((ContentView) mTarget).getChildAt(0).canScrollVertically(-1);
//        }
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        if (!isEnabled() || mReturningToStart || canChildScrollUp()
            ...
            // Chameleon change: We always want to cancel swipe on UP event
            if((action == MotionEvent.ACTION_UP || action == MotionEvent.ACTION_CANCEL)
                    && canChildScrollUp() && swipeState.swiping) {
                mOnProgressListener.onSwipeCancel();
                swipeState.setSwiping(false);
            }
    ...
    void setTargetOffsetTopAndBottom(int offset) {
        ...
        if(mOnProgressListener != null){
            float progress = mCurrentTargetOffsetTop/(mTotalDragDistance - mOriginalOffsetTop);
